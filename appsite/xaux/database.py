...
def insert_many(objects, ignore=False, using="default"):
	...
	if ignore:
		con.cursor().executemany(
			"insert ignore into %s (%s) values (%s)" % (table, column_names, placeholders), parameters)
	else:
		con.cursor().executemany(
			"insert into %s (%s) values (%s)" % (table, column_names, placeholders), parameters)
...
def update_many(objects, fields=[], using="default"):
	...
	con.cursor().executemany(
		"update %s set %s where %s=%%s" % (table, assignments, con.ops.quote_name(meta.pk.column)), parameters)
