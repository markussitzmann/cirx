...
class Database(models.Model):
	...
	url = models.ForeignKey('URL', blank=True, null=True, verbose_name="URL")
...
class Release(models.Model):
	...
	classification =  models.CharField(max_length=32, db_column='class', blank=True, choices = (('public','Public'),('private','Private'), ('internal','Internal',)))
...
class DatabaseDataCache:
	...
	def __init__(self):
		self.cached = {}
		self.cached['contexts'] = self.get_contexts()
		self.cached['urls'] = self.get_urls()
		self.cached['record_url_schemes'] = self.get_record_url_schemes()
		self.cached['organizations'] = self.get_organizations()
		self.cached['publishers'] = self.get_publishers()
		self.cached['databases'] = self.get_databases()
		self.cached['releases'] = self.get_releases()
	...
	def get_databases(self, database = None):
		try:
			...
			database_dict = {}
			if database:
				databases = Database.objects.filter(database = databases).values()
			else:
