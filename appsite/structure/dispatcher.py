...
class URLmethod:
	...
	def __init__(self, representation, request = None, parameters = None, output_format = 'plain'):
		type = ResponseType.objects.get(url = representation)
		self.type = type
		self.base_mime_type = type.base_mime_type
	...
	def names(self, string):
		...
		for interpretation in interpretations:
			...
			for structure in interpretation.structures:
				...
				names = []
				classification_strings = NameClassification.objects.all()
	...
	def chemspider_id(self, string):
		...
		for interpretation in interpretations:
			chemspider_id_list = []
			chemspider = ExternalResolver(name = "chemspider", url_scheme="http://www.chemspider.com/inchi-resolver/REST.ashx?q=%s\&of=%s")
			for structure in interpretation.structures:
				ens = structure.ens
