...
class String:
    ...
    def _test_prefix_inchi(self, string, prefix=DEFAULT_PREFIX):
        pattern_string = '^(?P<prefix>%s)(?P<version>.{1,2})/(?P<layers>.+$)' % prefix
...
class Key:
    ...
    def _set(self, key=None, prefix=DEFAULT_PREFIX, layer1=None, layer2=None, layer3=None, version=None):
        ...
        if key and not layer1 and not layer2 and not layer3:
            resolver_list = ['_test_prefix_standard_inchikey', '_test_standard_inchikey', '_test_long_prefix_inchikey',
                             '_test_short_prefix_inchikey', '_test_long_inchikey', '_test_short_inchikey']
            arg = "%s" % key
        elif layer1 and layer2 and not layer3 and not key:
            resolver_list = ['_test_long_prefix_inchikey', '_test_short_prefix_inchikey', '_test_long_inchikey',
                             '_test_short_inchikey']
    ...
    def _test_prefix_standard_inchikey(self, key, prefix=DEFAULT_PREFIX):
        pattern_string = '(?P<prefix>^%s)(?P<layer1>[A-Z]{14})-(?P<layer2>[A-Z]{8}S[A-Z]{1})-(?P<layer3>[A-Z]{1}$)' % prefix
        pattern = re.compile(pattern_string)
    ...
    def _test_standard_inchikey(self, key, prefix=DEFAULT_PREFIX):
        pattern_string = '(?P<layer1>[A-Z]{14})-(?P<layer2>[A-Z]{8}S[A-Z]{1})-(?P<layer3>[A-Z]{1}$)'
        pattern = re.compile(pattern_string)
        match = pattern.search(key)
        if match:
            identifier = match.groupdict()
    ...
    def _test_long_prefix_inchikey(self, key, prefix=DEFAULT_PREFIX):
        pattern_string = '(?P<prefix>^%s)(?P<layer1>[A-Z]{14})-(?P<layer2>[A-Z]{10}$)' % prefix
        pattern = re.compile(pattern_string)
        match = pattern.search(key)
        if match:
            identifier = match.groupdict()
    ...
    def _test_short_prefix_inchikey(self, key, prefix=DEFAULT_PREFIX):
        pattern_string = '(?P<prefix>^%s)(?P<layer1>[A-Z]{14}$)' % prefix
        pattern = re.compile(pattern_string)
        match = pattern.search(key)
        if match:
            identifier = match.groupdict()
